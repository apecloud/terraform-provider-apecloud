...
def load_environment(version: str) -> Environment:
    ...
    def check_and_add_model(name):
        if name in generated_models:
            return True
        generated_models.add(name)
        return False
    env.globals["model_already_generated"] = check_and_add_model
...
def load_templates(env: Environment) -> dict[str, Template]:
    templates = {
        "resource_model": env.get_template("resource_model.j2"),
        "datasource_model": env.get_template("datasource_model.j2"),
        "resource_schema": env.get_template("resource_schema.j2"),
        "datasource_schema": env.get_template("datasource_schema.j2"),
    }
    return templates
