...
def is_primitive(schema, depth=0):
    if depth > 10:
        return False
    if hasattr(schema, "get"):
        if schema.get("x-terraform-type") == "dynamic" or schema.get("x-terraform-recursion") is True:
            return True
        ...
        if schema.get("type") == "array":
            ...
            if hasattr(items, "get"):
                if items.get("x-terraform-type") == "dynamic" or items.get("x-terraform-recursion") is True:
                    return True
                if items.get("type") in PRIMITIVE_TYPES:
                    return True
    return False
